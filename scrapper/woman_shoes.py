...
m.make_sure_path_exists('woman_shoes')
html = urlopen(path+'/c/15/shoes-women/')
...
try:
    for link in bsObj1.find("ul", {"class":"cat-nav dt102_1"}).find("ul", {"class":"cat-nav cat-nav-sub dt102_2"}).findAll("li", {"class":"cat-nav-item dt102_li2"}):
        ...
        m.make_sure_path_exists('woman_shoes/'+result.contents[0])
        ...
        for link in bsObj2.find("div", {"class":"products-catalog__list"}).findAll("div", {"class":"products-list-item"}):
            if 'data-gallery' in link.attrs:
                ...
                for i in range(len(r)):
                    urlretrieve('https:'+r[i], m.getImagesDir()+'woman_shoes/'+result.contents[0]+'/'+result.contents[0]+'.'+str(j)+'.'+str(i)+'.jpeg')
                j += 1
            else:
                print(link.attrs)
                raise
